...
void MeanShift::shift_point(const Point &point,
    ...
    for(int i=0; i<points.size(); i++){
      std::cout << "looping pointzzatI" << i << '\n' << "size::" << points.size()<< '\n';

        const Point& temp_point = points.at(i);
        ...
        for(int j=0; j<shifted_point.size(); j++){
          std::cout << "looping pointzzatI" << j << '\n' << "size::" << shifted_point.size()<< '\n';
