...
void main( void ) {
		...
		// y軸曲げる
		float x = sin(p.y * 11.0  * 222.0);
	        float bx = cos(p.x * 1.0 +  222.0);
	        float gx = sin(p.x * 0.4 + time * 1.5);
	        float yx = sin(p.y * 2.0 + 40.0);
	...
	// x軸の設定（yをたす）？
		...
		vec3 destColor = rColor * y ;
	        ...
	        vec3 destColor2 = bColor * inversesqrt(rColor);
